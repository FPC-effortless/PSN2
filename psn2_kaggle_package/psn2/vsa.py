...
def bind(a: torch.Tensor, b: torch.Tensor, method: str = "multiply"):
    """
    VSA binding operation.
    
    Methods:
    - "multiply": Element-wise multiplication (fast, works for bipolar)
    - "circular": Circular convolution (proper VSA for continuous vectors)
    
    For normalized continuous vectors, circular convolution is the correct
    binding operation that allows proper unbinding via circular correlation.
    """
    if method == "circular":
        # Circular convolution via FFT
        return torch.fft.ifft(torch.fft.fft(a) * torch.fft.fft(b)).real
    else:
        # Element-wise multiplication (default for backward compatibility)
        return a * b

def unbind(c: torch.Tensor, b: torch.Tensor, method: str = "multiply"):
    """
    VSA unbinding operation (inverse of bind).
    
    For circular convolution: unbind via circular correlation (conjugate in freq domain)
    For multiplication: unbind via division (unstable) or multiplication (approximate)
    """
    if method == "circular":
        # Circular correlation: conj(FFT(b)) in frequency domain
        b_fft = torch.fft.fft(b)
        c_fft = torch.fft.fft(c)
        return torch.fft.ifft(c_fft * torch.conj(b_fft)).real
    else:
        # For multiplication binding, unbind is approximate (multiply again)
        # This only works for bipolar vectors where b*b=1
        return c * b
