...
class PhaseController:
    ...
    def run_pulse(self, external_input: torch.Tensor,
                  ...
                  regime: str = None) -> Optional[torch.Tensor]:
        """Execute one full A-F pulse. Returns committed_shape."""
        if self.budget <= 0:
            return self.committed_shape

        # Fix: Update active_regime from caller so bond formation works
    ...
    def _phase_b(self):
        ...
        # Fix: Apply local update per-sample in batch for dual-mode learning
        LR_FF = 1e-4
        ...
        if inp is not None:
            with torch.no_grad():
                ...
                if active_mask.any():
                    ...
                    if inp.dim() == 1:
                        inp = inp.unsqueeze(0)  # [1, D]
                    # Apply local update per sample in batch
