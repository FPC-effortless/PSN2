...
class GrowthLedger:
    ...
    def compute_prune_score(self, node_bank: "NodeBank", node_id: int,
        ...
        """
        prune_score = (1 - mean_activation) * sigmoid(age - tau_age_prune) + stuck_penalty

        Fix: Use tau (temporal trace) as primary activity signal, not e (prediction error).
        - tau tracks recent activation history (high = recently active, keep it)
        - e is prediction error (high = learning OR stuck; need age to distinguish)
        - Stuck nodes: high e for long time without tau increase = prune candidate
        """
        # tau is the correct activity measure: high tau = recently active
        tau_val = float(node_bank.tau[node_id].item())
        activity = min(1.0, tau_val / 5.0)  # 0 = never active, 1 = very active
        ...
        # Stuck penalty: persistently high error without learning (high e, low tau, old age)
        e_val = float(node_bank.e[node_id].item())
        stuck_penalty = 0.0
        if e_val > 1.0 and tau_val < 1.0 and age > 100:
            stuck_penalty = 0.3  # Strong signal to prune stuck nodes

        return float(inactivity * age_factor + stuck_penalty)
